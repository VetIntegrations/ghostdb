...
class ClientAction(base.BaseActionSet):
    ...
    create = base.ActionFactory(
        ...
        event_factory=base.EventFactory(
            event.InternalEvent,
            event.EVENT_RECORD_CREATE,
            data_dumper.GenericDataDumper
        )
    )
    ...
    add_contact = base.ActionFactory(
        ...
        event_factory=base.EventFactory(event.InternalEvent, event.EVENT_RECORD_CREATE, ClientRelatedDataDumper)
    )
    update_contact = base.ActionFactory(
        update_act.ContactUpdate,
        event_factory=base.EventFactory(event.InternalEvent, event.EVENT_RECORD_UPDATE, ClientRelatedDataDumper)
    )
    remove_contact = base.ActionFactory(
        delete_act.ContactDelete,
        event_factory=base.EventFactory(event.InternalEvent, event.EVENT_RECORD_DELETE, ClientRelatedDataDumper)
    )
