...
class TestKPIValueUpdate:
    ...
    def test_ok(self, dbsession, event_off):
        ...
        updated_kpi = dbsession.query(InternalKPIValue)[0]
    ...
    def test_update_right_record(self, dbsession, event_off):
        ...
        updated_kpi = dbsession.query(InternalKPIValue).filter(
            InternalKPIValue.id == self.kpi.id,

            InternalKPIValue.value == new_value
        )
        assert updated_kpi.count() == 1

        stay_kpi = dbsession.query(InternalKPIValue).filter(
            InternalKPIValue.id == kpi2.id,
            InternalKPIValue.value == kpi2.value,
            InternalKPIValue.kind == KPIKind.FINANCIAL_NET_REVENUE
        )
